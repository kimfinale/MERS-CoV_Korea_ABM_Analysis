...
### Parallel Simulation 
#### Simulation acorss threshold case for vaccination initiation
```{r}
source( "util/mers_ibm_funcs.R" )
# fit <- readRDS( "out/beta_move_1_4Dec2018.rds" )
# smpl <- sample( 1:nrow( fit$param ), 30 )
# fit_smpl <-data.frame( beta=fit$param[ smpl, 1 ], delay_move = fit$param[ smpl, 2 ] ) 

fit_subset <- readRDS( "out/pars_20181208T180504.rds" )
pars <- data.frame( beta = unique(fit_subset$beta), delay_move = unique(fit_subset$delay_move), n_per_par = 40 )
...
# 
# fit_smpl_long <- fit_smpl %>% dplyr::slice( rep( 1:n(), each = nrow(pars0) ) )
# 
# pars_long <- rbind( pars0, pars0 )
# for( i in 1:(nrow( fit_smpl )-2) ){
#   pars_long <- rbind ( pars_long, pars0 ) 
# }
# 
# pars <- cbind( fit_smpl_long, pars_long )
...
saveRDS( res_cls, paste0( "out/control_", tstamp, ".rds") )
...
```{r}
res <- readRDS( "out/vaccscenario_thresholdcase_20181208T180504.rds" )
pars <- readRDS( "out/pars_20181208T180504.rds" )
res_control <- readRDS( "out/control_20181209T231219.rds" ) 
# cumulative incidene under no vaccination for each set of beta and delay_move
med_inc_control <- sapply( 1:length(res_control), function(x) median( res_control[[x]]$cumul_symptom_onset ) )
n_set_other_par <- 15
med_inc_control <- med_inc_control[ rep(seq_len(length(med_inc_control)), each=n_set_other_par) ]
# overall vaccine effectiveness
vacc_occurred <- sapply( 1:nrow(pars), function(x) (sum(res[[x]]$vacc_day_adjusted)- pars$n_per_par[1]) )

ove <- sapply( 1:nrow(pars), function(x) 100 * ( 1 - median(res[[x]]$cumul_symptom_onset) / med_inc_control[x] ) )
vacc <- sapply( 1:nrow(pars), function(x) median( res[[x]]$cumul_vacc_dose ) )
case_averted <- sapply( 1:nrow(pars), function(x) 10000 * ( ( med_inc_control[x] - median( res[[x]]$cumul_symptom_onset )  )  / median( res[[x]]$cumul_vacc_dose ) ) ) #

df <- data.frame( ove = ove, case_averted = case_averted, vacc = vacc )
resdf <- cbind( pars, df )

library(dplyr)
ove_summary <- resdf %>% 
  group_by( vacc_scenario, threshold_case_vacc ) %>% 
  summarize( median = quantile( ove, probs=0.5 ),
             lower = quantile( ove, probs=0.025 ),
             upper = quantile( ove, probs=0.975 ) )
vacc_summary <- resdf %>% 
  group_by( vacc_scenario, threshold_case_vacc ) %>% 
  summarize( median = quantile( vacc, probs=0.5 ),
             lower = quantile( vacc, probs=0.025 ),
             upper = quantile( vacc, probs=0.975 ) )

caseaverted_summary <- resdf %>% 
  group_by( vacc_scenario, threshold_case_vacc ) %>% 
  summarize( median = quantile( case_averted, probs=0.5 ),
             lower = quantile( case_averted, probs=0.025 ),
             upper = quantile( case_averted, probs=0.975 ) )
...
library(ggplot2)
ggplot( resdf ) +
  geom_boxplot( aes(as.factor(threshold_case_vacc), 
                     ove, fill=as.factor(vacc_scenario) ), 
                 position = position_dodge(width = .6) ) +
  coord_cartesian( ylim=c(-100,100) ) +
  geom_hline( yintercept = 70, linetype="dashed" ) +
  labs( x="Incidence when vaccinnation started", y="Reduction in outbreak size (%)", 
         fill = "Vaccination\nscenario" ) +
  theme_classic() +
  theme( panel.grid.major = element_line( colour="#f0f0f0" ) ) + 
  annotate( "text", x=4.8, y=76, label = "Vaccine efficacy", size=3)
   
ggsave( "figs/vacc_cov_red_med_par_vary.png", width=3.4*1.5, height=2.5*1.5, units="in" )

ggplot( resdf ) +
   geom_boxplot( aes(as.factor(threshold_case_vacc), case_averted, fill=as.factor(vacc_scenario) ),
                 position = position_dodge(width = .6) ) + 
   coord_cartesian(ylim=c(-25,25)) +
   labs( x="Incidence when vaccinnation started", y="Case averted per 10,000 doses") +
   theme_classic() +
   theme( panel.grid.major = element_line( colour="#f0f0f0" ) ) +
  labs(fill='vaccination\nscenario')

ggsave( "figs/vacc_cov_case_med_par_vary.png", width=3.4*1.5, height=2.5*1.5, units="in" )

ggplot( resdf ) +
  geom_boxplot( aes(as.factor(threshold_case_vacc), 
                     vacc, fill=as.factor(vacc_scenario) ), 
                 position = position_dodge(width = .6) ) +
  labs( x="Incidence when vaccinnation started", y="Number of Vaccine doses used", 
         fill = "Vaccination\nscenario" ) +
  theme_classic() +
  theme( panel.grid.major = element_line( colour="#f0f0f0" ) )
   
ggsave( "figs/vacc_dose_med_par_vary.png", width=3.4*1.5, height=2.5*1.5, units="in" )
