...
fit_sample_long <- fit_sample %>% dplyr::slice( rep( 1:n(), each = nrow(pars0) ) )
...
#                     delay_move = fit_sample$delay_move, n_per_par = 40 )
...
index_Vacc_zero <- seq( 1, nrow(pars), by=6 ) # 6 comes from 0,0,1,0.3,...0,9
ci_vacc_zero_list <- lapply( index_Vacc_zero, function(x) matrix( rep(res[[x]]$cumul_symptom_onset, 6), nrow=length(res[[x]]$cumul_symptom_onset) ) )
...
iter <- 30 ## iterations acorss different random seeds during simulation
pars <- do.call( rbind, replicate( 30, pars, simplify=FALSE) ) 
resdf <-  cbind( pars, df )

## plot variance of medican across parameter estimates
library(tidyverse)
ggplot( filter( resdf, rel_pep == 0.5 ) ) +
...
##### Vaccince coverage and relative efficacy of PEP 
```{r}
## 30 parametrer sets and 30 runs per parameter set 
res <- readRDS( "out/vacccov_relpop_20181212T090546.rds" )
pars <- readRDS( "out/pars_20181212T090546.rds" )
...
ci_control <- ci_control[ rep( 1:length( ci_control ), each=6 ) ]
# overall vaccine effectiveness
# vacc_occurred <- sapply( 1:nrow(pars), function(x) (sum(res[[x]]$vacc_day_adjusted)- pars$n_per_par[1]) )
...
## plot variance of medican across parameter estimates
library(tidyverse)
ggplot( filter( resdf, rel_pep == 0.1 ) ) +
  geom_boxplot( aes(as.factor(100*vacc_cov), ove, fill=as.factor(100*vacc_eff) ),
                position = position_dodge(width = .6) ) +
  coord_cartesian( ylim=c(-100,100) ) +
  # geom_hline( yintercept = 70, linetype="dashed" ) +
