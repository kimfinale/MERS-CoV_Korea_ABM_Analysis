...
r <- 0.013 # ratio of low-risk person's infectivity to that of high-risk person
p <- 0.12 # prop of high-risk pop

d <- 4.259
R0 <- d*beta*((1-p) + p*(1/r))
quantile(R0,probs=c(0.025,0.5,0.975))  
  
```

### Time series based on posterior distributions from ABC
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
source( "util/mers_ibm_funcs.R" )
library( dplyr )
# fit <- readRDS( "out/beta_delaymove_20181216T235939.rds" )
fit = readRDS( "out/fit_20190123T195146.rds" )
fit <- data.frame( beta = fit$param[,1], delay_move = fit$param[,2] )
fit_sample <- fit[ sample( nrow(fit), size = 100 ), ] 
...
pars0 <- data.frame( expand.grid( 
  outbreak_scenario = outbreak_scenario,
  under_vacc = under_vacc,
  n_per_par = n_per_par,
  rng_seed_init = rng_seed_init ), stringsAsFactors = FALSE )
...
res_cls <- pblapply( 1:nrow(pars),
                      FUN = function(x) run_java_ibm(
                         rng_seed_init = pars$rng_seed_init[ x ],
...
library( dplyr )
# fit <- readRDS( "out/beta_delaymove_20181216T235939.rds" )
fit <- readRDS( "out/fit_20190123T123646.rds" )
fit_sample <- data.frame( beta = fit$param[,1],
                          delay_move = fit$param[,2] )
fit_sample <- unique( fit_sample )
...
stopCluster( cl )
tstamp <- format( Sys.time(), "%Y%m%dT%H%M%S" )
saveRDS( res_cls, paste0( "out/thresholdcase_", tstamp, ".rds") )
saveRDS( pars, paste0( "out/pars_par_",tstamp, ".rds" ) )
Sys.time()-tbegin
...
stopCluster( cl )
tstamp <- format( Sys.time(), "%Y%m%dT%H%M%S" )
saveRDS( res_cls, paste0( "out/vacccov_vacceff_", tstamp, ".rds") )
saveRDS( pars, paste0( "out/pars_",tstamp, ".rds" ) )
Sys.time()-tbegin

```

### Vaccine impact under importation-driven outbreaks
```{r}
source( "util/mers_ibm_funcs.R" )
library( dplyr )
fit <- readRDS( "out/beta_delaymove_20181216T235939.rds" )
