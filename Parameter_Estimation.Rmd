...
### Del Moral et al. sequential Monte Carlo approximate Bayesian computation
```{r}
rm(list=ls())
library(rJava)
...
prior = list( c("unif", 1e-3, 1), c("unif", 1e-3, 10) )
alpha = 0.5
tolerance = 1e-5

tbegin = proc.time()
fit = ABC_sequential( method="Delmoral", 
                      model = run_ibm_abc_cls, 
                      prior = prior,
                      nb_simul = 1000, 
...
hist( fit$param[,1], breaks=seq(0,1,0.1) )
hist( fit$param[,2], breaks=seq(0,10,0.1) )

tstamp <- format( Sys.time(), "%Y%m%dT%H%M%S" )
saveRDS( fit, paste0( "out/fit_", tstamp, ".rds") )
...
toy_model2_1<-function(x){ 
   set.seed ( x[ 1 ] )
   summary_stat <- c( x[2] + x[3] + rnorm(1,0,0.1) - 1.5, 
                      x[2] * x[3] + rnorm(1,0,0.1) - 0.5 ) 
}
toy_prior2=list(c("unif",0,10),c("unif",0,10))
sum_stat_obs2=c(0,0)
tolerance=0.001
...
abc_beaumont <- ABC_sequential( method="Delmoral", model=toy_model2_1, 
                                prior=toy_prior2, nb_simul=10-0, 
                                summary_stat_target=sum_stat_obs2, 
                                tolerance=tol, 
                                progress_bar=TRUE, 
                                use_seed=TRUE, n_cluster=3 )
